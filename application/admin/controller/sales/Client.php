...
class Client extends Backend
{
    ...
    public function index()
    {
        ...
        if ($this->request->isAjax())
        {
            ...
            foreach ($list as $row) {
                $row->visible(['id','logo','short_name','type','star','website','status','updatetime']);
                $row->visible(['category']);
				$row->getRelation('category')->visible(['name']);
				$row->visible(['admin']);
				$row->getRelation('admin')->visible(['nickname']);
				$row->visible(['country']);
				$row->getRelation('country')->visible(['name','timezone']);
            }
        ...
        if ($this->request->isPost()) {
            ...
            if ($params) {
                ...
                try {
                    ...
                    Hook::listen('after_write_client', $this->model);
                    Db::commit();
        ...
        if ($this->request->isPost()) {
            ...
            if ($params) {
                ...
                try {
                    ...
                    Hook::listen('after_write_client', $row);
                    Db::commit();
                } catch (ValidateException $e) {
                    Db::rollback();
                    $this->error($e->getMessage());
